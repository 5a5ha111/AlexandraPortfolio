...
half4 GetFinalBaseColor(Varyings input)
{
    if (_IsTextureRef)
    {
        half4 texBrightMap = tex2D(_BaseMap, input.uv);
        half texBright = (texBrightMap.r + texBrightMap.b + texBrightMap.g) / 3;
        texBright = max(max(texBrightMap.r, texBrightMap.b), texBrightMap.g);
        half refBright =  (_BrightestTextureColor.r + _BrightestTextureColor.g + _BrightestTextureColor.b) / 3;
        refBright = max(max(_BrightestTextureColor.r, _BrightestTextureColor.g), _BrightestTextureColor.b);
        float colorMult = Unity_Remap_float(texBright, float2(0, refBright), float2(0, 1));
        return colorMult * _BaseColor;
    }
...
half4 ShadeFinalColor(Varyings input) : SV_TARGET
{
    ...
    //return  GetFinalBaseColor(input);
    return half4(color, surfaceData.alpha);
}
