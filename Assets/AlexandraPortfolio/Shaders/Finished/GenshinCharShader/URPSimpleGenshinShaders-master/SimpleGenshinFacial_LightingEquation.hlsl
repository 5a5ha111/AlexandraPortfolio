...
float Dot2(float2 vec1, float2 vec2)
{
...
half3 CalculateFaceShadowMapShading(ToonSurfaceData surfaceData, ToonLightingData lightingData, Light light)
{
    ...
    //return litOrShadow;
    float3 litOrShadow = step((-FdotL + 1) / 2, faceShadowMap);
    float3 _Transform_Out = TransformObjectToWorld(half3(0,0,1)); //Face forward dir. May be vary 
    float3 _Transform_Out2 = TransformObjectToWorld(half3(0,0,0));
    float3 FaceDir = normalize((_Transform_Out - _Transform_Out2));  
    float2 FaceDir2 = normalize(float2(FaceDir.x, FaceDir.z)); 
    float2 LightDir2 = normalize(float2(light.direction.x, light.direction.z));
    //float _OutRemap = Unity_Remap_float(Dot2(LightDir2, FaceDir2), float2(0,1), float2(-1,1));
    float _OutRemap = Dot2(LightDir2, FaceDir2);
    //return _OutRemap;
    float TuneDonwAngle = 0.025;
    if (_OutRemap > TuneDonwAngle)
    {
        //float tuneDownShadow = _OutRemap > TuneDonwAngle ? _OutRemap : TuneDonwAngle;
        float tuneDownShadow = 1 - Unity_Remap_float((_OutRemap), float2(1, 1 - TuneDonwAngle), float2(0, 1));
        float tuneDownShadowSmooth = smoothstep(0, 1, tuneDownShadow);
        //return tuneDownShadow;
        half NfaceShadowMap = lerp(faceShadowMap, 0, 1 - tuneDownShadow);
        half NlitOrShadow = step((-FdotL + 1) / 2, NfaceShadowMap);
        litOrShadow = litOrShadow < 0.5 ? lerp(NlitOrShadow, 1, tuneDownShadowSmooth) : 1;
        return litOrShadow;
    }
    if (_OutRemap < -1 + TuneDonwAngle)
    {
        //float tuneDownShadow = _OutRemap > TuneDonwAngle ? _OutRemap : TuneDonwAngle;
        float tuneDownShadow = 1 - Unity_Remap_float(abs(_OutRemap), float2(1, 1 - TuneDonwAngle), float2(0, 1));
        float tuneDownShadowSmooth = smoothstep(0, 1, tuneDownShadow);
        //return tuneDownShadow;
        half NfaceShadowMap = lerp(faceShadowMap, 1, tuneDownShadow);
        half NlitOrShadow = step((-FdotL + 1) / 2, faceShadowMap);
        litOrShadow = litOrShadow > 0.5 ? lerp(NlitOrShadow, 0, tuneDownShadow) : 0;
    }
    return litOrShadow;
    //return dot(LightDir, FaceDir);
    //return litOrShadow;
}
