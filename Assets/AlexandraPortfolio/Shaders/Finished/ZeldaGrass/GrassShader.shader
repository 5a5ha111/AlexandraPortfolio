Shader "SashaMakesGames/GrassShader"
{
    Properties
    {
        ...
        _TessellationGrassDistance("Tessellation Grass Distance", Range(0, 2)) = 0.1
    ...
    SubShader
    {
        ...
        HLSLINCLUDE
            ...
            float tessellationEdgeFactor(VertexInput vert0, VertexInput vert1)
            {
                ...
                return edgeLength / (_TessellationGrassDistance /*/ ObjectScale().x*/);
            }
            ...
            GeomData TransformObjectToClip(float3 pos, float3 offset, float3x3 transformationMatrix, float2 uv)
            {
                GeomData o;
            ...
            {
                float grassVisibility = tex2Dlod(_GrassMap, float4(input[0].uv, 0, 0)).a;

                if (grassVisibility >= _GrassThreshold + _GrassFalloff / BLADE_SEGMENTS)
                {
                    float4 pos = input[0].vertex;
                    float3 prePos_WS = TransformObjectToWorld(pos.xyz);
                    //pos = TransformObjectToWorld(prePos);
                    float3 faceNormal = float3(0, 1, 0);
                    float3 noOffset = float3(0.0, 0.0, 0.0);
                    ...
                    float2 windSample = (smoothstep(float2(0,0), float2(1,1), tex2Dlod(_WindMap, float4(windUV, 0, 0)).xy) * 2 - 1) * length(_WindVelocity);
                    ...
                    float width  = lerp(_BladeWidthMin * falloff, _BladeWidthMax, rand(pos.xzy) * falloff);
                    float height = lerp(_BladeHeightMin * falloff, _BladeHeightMax, rand(pos.zyx) * falloff);
                    //height = height * expStep(0.1, 1, grassVisibility + _GrassFalloff + _GrassThreshold);
                    ...
                    for (int i = 0; i < BLADE_SEGMENTS; ++i)
                    {
                        ...
                        /*triStream.Append(TransformObjectToClip(pos, float3( offset.x, offset.y, offset.z), transformationMatrix, float2(0, t)));
                        triStream.Append(TransformObjectToClip(pos, float3(-offset.x, offset.y, offset.z), transformationMatrix, float2(1, t)));*/


                        /*float3 prePos1_WS = prePos_WS + float3( offset.x, offset.y, offset.z);
                        float3 prePos2_WS = prePos_WS + float3(-offset.x, offset.y, offset.z);*/
                        float3 prePos1_WS = prePos_WS + float3( offset.x, offset.z, offset.y);
                        float3 prePos2_WS = prePos_WS + float3(-offset.x, offset.z, offset.y);

                        float3 pos1_OS = TransformWorldToObject(prePos1_WS);
                        float3 pos2_OS = TransformWorldToObject(prePos2_WS);
                        
                        triStream.Append(TransformObjectToClip(pos1_OS, noOffset, transformationMatrix, float2(0, t)));
                        triStream.Append(TransformObjectToClip(pos2_OS, noOffset, transformationMatrix, float2(1, t)));
                    }

                    // Add the final vertex at the tip of the grass blade.
                    //triStream.Append(TransformObjectToClip(pos, float3(0, forward, height), tipTransformationMatrix, float2(0.5, 1)));

                    //float3 prePos3_WS = prePos_WS + float3(0, forward, abs(height));
                    float3 prePos3_WS = prePos_WS + float3(0, height, forward);
                    float3 pos3_OS = TransformWorldToObject(prePos3_WS);
                    triStream.Append(TransformObjectToClip(pos3_OS, noOffset, tipTransformationMatrix, float2(0.5, 1)));
            ...
            HLSLPROGRAM
                ...
                float4 frag(GeomData i) : SV_Target
                {
                    ...
                    //#ifdef _MAIN_LIGHT_SHADOWS
                        ...
                        half shadowAttenuation = saturate(MainLightRealtimeShadow(TransformWorldToShadowCoord(i.worldPos)) /*+ 0.25f*/);
                        float4 shadowColor = lerp(0.3f, 1.0f, shadowAttenuation);
                        color *= shadowColor;
                    //#endif
                    
                    color = color * lerp(_BaseColor, _TipColor, i.uv.y);
                    color.a = lerp(0, 1, saturate(i.uv.y * 4));
                    return color;
                }
            ENDHLSL
        }
