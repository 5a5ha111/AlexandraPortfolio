...
public class GrassComputeScript : MonoBehaviour
{
    ...
    public ComputeShader BackFaceCullCompute;
    private ComputeShader m_InstancedBackFaceCullCompute;
    private ComputeBuffer m_CulledBuffer;
    ...
    List<int> grassVisibleIDListCached = new List<int>();
    ...
    private uint[] argsBufferReset = new uint[5]
   {
    ...
    void MainSetup(bool full)
    {
        ...
        m_InstantiatedMaterial.EnableKeyword("_MAIN_LIGHT_SHADOWS");
        m_InstantiatedMaterial.EnableKeyword("_MAIN_LIGHT_SHADOWS_CASCADE");
        m_InstantiatedMaterial.EnableKeyword("_SHADOWS_SOFT");
        m_InstantiatedMaterial.EnableKeyword("LIGHTMAP_ON");
        m_InstantiatedMaterial.EnableKeyword("_ADDITIONAL_LIGHTS");
        m_InstantiatedMaterial.EnableKeyword("_ADDITIONAL_LIGHT_SHADOWS");

        m_InstancedBackFaceCullCompute = Instantiate(BackFaceCullCompute);
        ...
        m_CulledBuffer = new ComputeBuffer(maxBufferSize, DRAW_STRIDE, ComputeBufferType.Append);

        // Cache the kernel IDs we will be dispatching
        m_IdGrassKernel = m_InstantiatedComputeShader.FindKernel("Main");
        m_IdCullKernel = m_InstancedBackFaceCullCompute.FindKernel("CSMain");
        ...
        m_InstancedBackFaceCullCompute.SetInt("_NumSourceVertices", numSourceVertices);
        //Debug.Log("m_MainCamera.transform " + m_MainCamera.transform);
        //var _CameraTransform = m_MainCamera.transform.eulerAngles;
        //_CameraTransform = new Vector3(_CameraTransform.x - 180, _CameraTransform.y - 180, _CameraTransform.z - 180);
        //Vector3 _Cameradir = new Vector3(_CameraTransform.x, _CameraTransform.y, _CameraTransform.z);
        //m_InstancedBackFaceCullCompute.SetVector("_Camera_Direction", m_MainCamera.transform.rotation.eulerAngles);
        if (m_MainCamera != null)
        {
            m_InstantiatedComputeShader.SetVector("_Camera_Direction", m_MainCamera.transform.eulerAngles);
        }
        
        /*float dott = Vector3.Dot(new Vector3(0, 1, 0), _Cameradir.normalized);
        Debug.Log("rotation.eulerAngles " + _CameraTransform.normalized);
        Debug.Log("rotation.eulerAngles " + _CameraTransform);
        Debug.Log("dott " + dott);*/
        // cache shader property to int id for interactivity;
        shaderID = Shader.PropertyToID("_PositionsMoving");

        m_InstancedBackFaceCullCompute.SetBuffer(m_IdCullKernel, "_DrawTriangles", m_DrawBuffer);
        m_InstancedBackFaceCullCompute.SetBuffer(m_IdCullKernel, "_DrawTrianglesOut", m_CulledBuffer);
    ...
    private void OnDisable()
    {
        ...
        if (m_Initialized)
        {
            ...
            if (Application.isPlaying)
            {
                ...
                Destroy(m_InstancedBackFaceCullCompute);
            }
            else
            {
                DestroyImmediate(m_InstantiatedComputeShader);
                DestroyImmediate(m_InstantiatedMaterial);
                DestroyImmediate(m_InstancedBackFaceCullCompute);
            }
            // Release each buffer
    ...
    private void Update()
    {
        ...
        m_CulledBuffer.SetCounterValue(0);
        ...
        if (m_DispatchSize > 0)
        {
            ...
            //Debug.Log("m_DrawBuffer.count " + m_DrawBuffer.count);
            //Debug.Log("SystemInfo.maxGraphicsBufferSize " + SystemInfo.maxGraphicsBufferSize / 1024);

            //m_InstancedBackFaceCullCompute.Dispatch(m_IdCullKernel, m_DispatchSize, 1, 1);
            //Debug.Log("Cull");
